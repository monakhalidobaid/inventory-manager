...
if (cat_name) {
    errorMessage = cat_name.nextElementSibling;
}
...
function validateDepartmentName(name) {
    ...
    if (value === "") {
        ...
        return "The category name must be at least two letters long.";
    }
    return ""; 
}
...
function displayCat(cats) {
  ...
  cats.forEach(cat => {
    ...
    td1.textContent = cat.cat_id;

    let td2 = document.createElement('td');
    td2.textContent = cat.cat_name;
    ...
    if (window.userType === 'admin') {
      ...
      td3.textContent = ""; // مستخدم عادي: بدون خيارات
    }
...
form.onsubmit = async function(e) {
  e.preventDefault();
      ...
      let error = validateDepartmentName(cat_name.value);

if (error) {
    e.preventDefault();
    errorMessage.textContent = error;
    errorMessage.classList.add("show");       // تظهر الرسالة تدريجياً
    cat_name.classList.add("input-error");   // الحدود تتحول للأحمر تدريجياً
            return; // يمنع متابعة تنفيذ الكود

} else {
    errorMessage.textContent = "";
    errorMessage.classList.remove("show");    // تختفي تدريجياً
    cat_name.classList.remove("input-error");

}

  const catNameValue = document.getElementById("cat_name").value.trim();

    const catData = { catNameValue: catNameValue }; 
    console.log(catData)

  try {
    const response = await fetch('../api/category/insert.php', {
      method: 'POST',
...
//fetch cat with paging
async function loadCategory(search = '', page = 1, limit = 5) {
  try {
    ...
    const url = `../api/category/get.php?${params.toString()}`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (searchInput) {
    searchInput.addEventListener("input", debounce((e) => {
      ...
      loadCategory(query, 1); // دايم يبدأ من الصفحة 1
    }, 300));
  }
...
document.addEventListener("click", function(e) {
  if (e.target.closest(".edit")) {
    ...
    currentCatId = parseInt(row.cells[0].textContent.trim(), 10);
    let catName = row.cells[1].textContent;

//  تنظيف أي رسائل قديمة قبل فتح المودل
editErrorMessage.textContent = "";
editErrorMessage.classList.remove("show");
editName.classList.remove("input-error");

    editName.value = catName;
...
window.addEventListener("click", function(e) {
  if (e.target === editModal) {
    ...
    //  تنظيف الخطأ لما يقفل المودل بالضغط بره
    editErrorMessage.textContent = "";
    editErrorMessage.classList.remove("show");
    editName.classList.remove("input-error");
  }
...
editForm.onsubmit = async function(e) {
  ...
  if (nameValue === "") {
    editErrorMessage.textContent = "Please enter the category name";
    editErrorMessage.classList.add("show");
    editName.classList.add("input-error");
    return;
  } else if (nameValue.length < 2) {
    editErrorMessage.textContent = "The category name must be at least two letters long.";
    editErrorMessage.classList.add("show");
    editName.classList.add("input-error");
    return;
  } else {
...
document.addEventListener("click", function(e) {
  if (e.target.closest(".delete")) {
    e.preventDefault();
    let row = e.target.closest("tr");
    catIdToDelete = parseInt(row.cells[0].textContent.trim(), 10);
...
confirmDeleteBtn.addEventListener("click", async () => {
  ...
  try {
    const response = await fetch("../api/category/delete.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: catIdToDelete })
    });
