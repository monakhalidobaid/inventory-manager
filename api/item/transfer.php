...
// فحص 1: التأكد من أن العنصر معين لقسم على الأقل
if (!$currentItem['dept_id']) {
    echo json_encode([
        'success' => false,
        'message' => 'Cannot transfer unassigned item. Please use Assign first.'
    ]);
    exit;
}

// فحص 2: التأكد من أن القيم الجديدة مختلفة عن القيم الحالية
$old_dept_id = $currentItem['dept_id'];
$old_emp_id = $currentItem['emp_id'];

// مقارنة القيم
$isDeptSame = ($old_dept_id == $dept_id);
$isEmpSame = ($old_emp_id == $emp_id);

if ($isDeptSame && $isEmpSame) {
    echo json_encode([
        'success' => false,
        'message' => 'No changes detected. Item is already assigned to the selected department and employee.'
    ]);
    exit;
}
...
try {
    ...
    // بناء رسالة النجاح
    $message = $emp_id 
        ? "Item successfully transferred to $empName ($deptName)" 
        : "Item successfully transferred to $deptName department";
